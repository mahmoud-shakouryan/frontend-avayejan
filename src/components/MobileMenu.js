  ...
  return (
    <div className="z-50">
      <ul
        className={
          activeSideMenu
            ? "font-secondFont text-sm fixed right-0 top-0 h-screen w-1/2  bg-dark flex  flex-col justify-start items-center md:hidden"
            : "hidden"
        }
      >
        <li
          onClick={showMenu}
          className=" p-5 w-full flex items-center justify-center sm:hover:text-shade cursor-pointer"
        >
          <CloseOutlinedIcon className="scale-150 flex items-center justify-center" />
        </li>
        <li
          onClick={showMenu}
          className="w-full flex items-center justify-center sm:hover:text-shade"
        >
          <NavLink
            to="/"
            className={({ isActive }) =>
              isActive
                ? "p-5 w-full flex items-center justify-center text-orange"
                : "p-5 w-full flex items-center justify-center"
            }
          >
            صفحه اصلی
          </NavLink>
