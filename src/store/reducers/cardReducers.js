...
const cardReducerInState =  { cardItems: localStorage.getItem('cardItems') ? JSON.parse(localStorage.getItem('cardItems')).cardItems : [] };
...
export const cardReducer = (state = cardReducerInState, action ) => {
    ...
    switch(action.type){
        case actions.ADD_TO_CARD:
            ...
            const existItem = state.cardItems.find( cardItem => cardItem.id === item.id ); 
            if(existItem){
